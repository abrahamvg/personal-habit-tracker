...
export default function DashboardTaskCard({ habit, onToggle, isPinned }: DashboardTaskCardProps) {
  ...
  return (
    <div className="relative group">
      <div className="card p-5 hover:shadow-lg transition-all duration-300 border-2 border-transparent hover:border-ocean-300 dark:hover:border-ocean-700">
        <div className="flex items-start gap-4">
          {/* Checkbox */}
          <button
            onClick={onToggle}
            className="flex-shrink-0 mt-1 w-8 h-8 rounded-full border-3 border-ocean-400 dark:border-ocean-600 hover:border-ocean-500 dark:hover:border-ocean-500 transition-all duration-200 flex items-center justify-center hover:scale-110 active:scale-95"
            aria-label="Complete task"
          >
            <CheckCircle2 className="w-5 h-5 text-ocean-400 dark:text-ocean-600 opacity-0 group-hover:opacity-100 transition-opacity" />
          </button>

          {/* Content */}
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 mb-2">
              <h3 className="font-semibold text-lg text-ocean-900 dark:text-dark-text-primary">
                {habit.name}
              </h3>
              {isPinned && (
